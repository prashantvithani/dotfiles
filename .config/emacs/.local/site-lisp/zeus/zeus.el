...
(defun zeus--available-commands ()
  (let* ((zeus-json (json-read-from-string
                     (with-temp-buffer
                       (insert-file-contents (expand-file-name "zeus.json" (doom-project-root)))
                       (buffer-string))))
...
simple-status output"
  (interactive)
  (setq *zeus-server-process* (zeus--run "zeus-server" "*zeus-server*" "start" "" '("--simple-status" "--log" "/dev/stdout")))
